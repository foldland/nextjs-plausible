...
A non-breaking feature contribution to `nextjs-plausible` could look like this:
```bash
git commit -m "feat(nextjs-plausible): Add a super cool feature."
```
You can read the full documentation at https://www.conventionalcommits.org.

## Monorepo
For easier development and potential future restructures (adding related packages) we use a monorepo structure.
This means that we have multiple packages in one git repository.
We use [pnpm](https://pnpm.io/workspaces) to manage the packages in this repository.
...
You can read more about it in the [biomejs docs](https://biomejs.dev/guides/getting-started/).
